...
class BaseBars(ABC):
    ...
    def __init__(self, metric: str, batch_size: int = 2e7):
        ...
        self.has_buyer_maker_flag = (
            False  # Flag for checking if has flag for buyer maker
        )
    ...
    def run(self, data: Union[list, tuple, pd.DataFrame]) -> list:
        ...
        # print(f"type of values is {type(values)}  {__file__} len: {len(values)}")
    ...
    def _assert_csv(test_batch: pd.DataFrame):
        ...
        try:
            ...
            raise ValueError(
                "csv file, column 0, not a date time format:", test_batch.iloc[0, 0]
            )
