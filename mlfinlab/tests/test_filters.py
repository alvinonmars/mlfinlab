...
from mlfinlab.filters.filters import cusum_filter, cusum_filter2, z_score_filter
    ...
    def test_cusum_filter(self):
        ...
        for threshold in [0.005, 0.007, 0.01, 0.015, 0.02, 0.03, 0.04]:
            for timestamp in [True, False]:
                cusum_events1 = cusum_filter(
                    self.data["close"], threshold=threshold, time_stamps=timestamp
                )
                cusum_events = cusum_filter2(
                    self.data["close"], threshold=threshold, time_stamps=timestamp
                )
                print(f"len(cusum_events1) = {len(cusum_events1)}")
                print(f"len(cusum_events) = {len(cusum_events)}")
